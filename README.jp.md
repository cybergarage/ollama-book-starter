...
cd ollama-book-starter/ja
```

`--depth 1` は履歴を取り込まず最新の内容だけを取得する指定です。内容を新しくするときはリポジトリ直下で `git pull` を実行してください。
...
**`ollama-book-starter/ja`から**実行してください。
...
書籍の章との対応は、この表だけを正とします。サンプルは`ja/`から実行します。
